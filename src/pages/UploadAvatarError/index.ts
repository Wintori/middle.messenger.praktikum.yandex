...
import template from './uploadAvatarError.hbs'
import backImage from '../../assets/button-back.svg'
import { Button } from "../../components/Button";
import { Input, InputProps } from "../../components/input/input";

interface UploadAvatarErrorPageProps {
    profileInputs: InputProps[],
    userName: string,
}

export class UploadAvatarErrorPage extends Block {
    constructor(props: UploadAvatarErrorPageProps) {
        super(props)
    }
    protected init(): void {
        ...
        this.children.buttonUpload = new Button({
            events: {
                click: (evt: PointerEvent) => {
                    evt.preventDefault();
                    window.location.pathname = '/profile/uploadAvatar/success'
                }
            },
